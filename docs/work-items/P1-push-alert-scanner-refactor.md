...
- 다음 작은 후보: scanner orchestration helper 또는 crypto/global scanner 분리를 동작 변경 없이 진행합니다. 발송 정책, `push_alert_events` DB 기록, 중복 방지 event key, 권한 판정, threshold 정책은 변경하지 않습니다.

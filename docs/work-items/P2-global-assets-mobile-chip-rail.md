...
- 2026-06-11: `/global/assets` 주요 자산 선택칩을 모바일에서 4열 grid로 표시하도록 정리했습니다. `sm` 이상에서는 기존 가로 흐름을 유지합니다.
- 수정 파일:
  - `src/components/global/GlobalAssetSelectionPanel.tsx`
  - `docs/work-items/active-backlog.md`
  - `docs/work-items/P2-global-assets-mobile-chip-rail.md`
- 검증:
  - `git diff --check`: PASS
  ...
  - Browser 360px급 viewport: `/global/assets` 재확인 PASS. 주요 자산 버튼 `NQ=F`, `ES=F`, `QQQ`, `SPY`, `^VIX`, `TLT`, `NVDA`, `SMH`, `GLD`, `CL=F`가 viewport 안에 배치됐고 horizontal overflow와 console error는 없었습니다.
- 완료 커밋: 미커밋
- PR:

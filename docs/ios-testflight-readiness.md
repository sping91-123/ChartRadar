...
## iOS Capacitor Platform Setup - Task 4 Platform Generation Preconditions
...
### Precondition Commands And Checks

| Check | Result |
| --- | --- |
| `git status --short` | Clean working tree before documentation edits. |
| `npm.cmd ls @capacitor/core @capacitor/android @capacitor/ios` | `@capacitor/core@8.3.3`, `@capacitor/android@8.3.3`, and `@capacitor/ios@8.3.3` are installed. |
| `dir mobile-shell` / recursive listing | `mobile-shell` exists and currently contains `index.html` only. |
| `dir ios` / existence check | `ios/` directory is still missing. |
| `capacitor.config.ts` source inspection | `appId` is `com.staronlabs.chartradar`, `appName` is `Chart Radar`, and `webDir` is `mobile-shell`. |
| `package.json` / `package-lock.json` source inspection | `@capacitor/ios` is declared and locked at the same `8.3.3` family as `@capacitor/core`, `@capacitor/android`, and `@capacitor/cli`. |

### WebDir Readiness

| Item | State |
| --- | --- |
| `mobile-shell` directory | Present. |
| Default HTML | `mobile-shell/index.html` is present. |
| Asset files | No separate asset files are present under `mobile-shell`; the shell is a minimal single-file placeholder. |
| Android precedent | Existing Capacitor config already points Android/common packaging at `mobile-shell`. |
| Build output note | TODO 3 validation ran `npm.cmd run build` successfully, but the Capacitor `webDir` remains the existing `mobile-shell` shell, not `.next` output. |
| Precondition assessment | Sufficient for the next TODO to attempt native iOS project generation with `npx cap add ios`; production runtime/content behavior still needs later native audit and launch validation. |

### Capacitor Config Readiness

| Setting | Current value | Precondition assessment |
| --- | --- | --- |
| `appId` | `com.staronlabs.chartradar` | Ready as the current Bundle ID candidate; do not change in this task. |
| `appName` | `Chart Radar` | Ready as the current display name candidate; do not change in this task. |
| `webDir` | `mobile-shell` | Directory exists and has `index.html`. |
| iOS-specific config | None found. | Not a blocker for `npx cap add ios`, but generated iOS project should be audited before any build/upload. |
| Conditional `server` config | Uses `CAPACITOR_SERVER_URL` when set and marks cleartext based on `http://`. | Requires later iOS review for App Store/runtime behavior; not changed here. |
| Android config | `allowMixedContent` and `captureInput` are Android-specific. | No change. |
| Push plugin config | Common presentation options exist. | iOS push still requires APNs/Firebase/capability follow-up. |
...
| `@capacitor/cli` | `8.3.3` |
| `@capacitor/ios` | `8.3.3` |
| Version alignment | Aligned on Capacitor `8.3.3` family. |
| Additional dependency needed before `npx cap add ios` | None identified from package inspection. |

### Next Command Candidate For Task 5

| Item | Candidate |
| --- | --- |
| Command | `npx cap add ios` |
| Expected primary change | New `ios/` directory generated by Capacitor. |
| Expected generated file families | Xcode project/workspace files, native app target files, `Podfile`, iOS resources, plist/config files, and supporting Capacitor native files. |
| Expected package changes | None expected from the command itself, but review the diff after execution. |

### Task 5 Guardrails

- Run only `npx cap add ios` unless the next task explicitly authorizes another command.
- Do not run `npx cap sync ios`, `npx cap open ios`, Xcode, manual `pod install`, `xcodebuild`, fastlane, archive, upload, or submission commands.
- Do not modify generated native files in the same task; inspect and document them after generation.
- Stop if the command changes files outside the expected `ios/` generated scope or package metadata.
...
## iOS Capacitor Platform Setup - Task 8 Next Follow-Up Selection
...
### iOS Platform Setup Final Summary

| Area | Final state |
| --- | --- |
| `@capacitor/ios` dependency | Added and aligned at `8.3.3`. |
| Native iOS project | Generated under `ios/`. |
| Bundle ID | `com.staronlabs.chartradar`. |
| Display name | `Chart Radar`. |
| Deployment target | `15.0`. |
| Native dependency model | Swift Package Manager setup generated by Capacitor. |
| Safe validation | `git diff --check`, TypeScript, build, `smoke:copy`, and `smoke:mobile` passed in Task 7. |
| Android/protected paths | No Android release, auth, Supabase, billing, RevenueCat, entitlement, push sending, or production configuration changes were made. |
| Ignored generated outputs | `ios/App/App/capacitor.config.json`, `ios/App/App/config.xml`, `ios/App/App/public/`, and `ios/capacitor-cordova-ios-plugins/` remain on disk but ignored. |

### Remaining Blockers

| Blocker | Why it matters |
| --- | --- |
| Signing / Team ID missing | Local build, archive, and TestFlight upload cannot proceed safely without Xcode signing readiness. |
| Apple Developer / Bundle ID / App Store Connect setup unverified | Capabilities, provisioning, and app record readiness are unresolved. |
| `.entitlements` file absent | Sign in with Apple, push, and other capabilities may require entitlement setup later. |
| Sign in with Apple risk HIGH | iOS review risk remains if third-party login is exposed without Apple sign-in handling. |
| RevenueCat / App Store product mapping incomplete | iOS paid features and restore behavior are not ready for purchase validation. |
| APNs / Firebase iOS push incomplete | iOS push token and notification delivery readiness remain unproven. |
| iOS build/archive/upload not attempted | Technical build path still needs a separate, guarded run. |
| Ignored generated output policy not finalized | Generated `public` and config outputs are ignored; future sync/build workflow should document recreation expectations. |

### Follow-Up Candidate Evaluation

| Candidate | Priority | Risk | Evaluation |
| --- | --- | --- | --- |
| `ios-xcode-signing-readiness-run` | 1 | HIGH | Best next blocker to audit because generated iOS project now exists, but Team ID, Bundle ID registration, provisioning, and capabilities remain unresolved before any build/upload path can be trusted. |
| `ios-auth-apple-signin-risk-run` | 2 | HIGH | Important App Store review risk, but Apple sign-in capability and Bundle ID/signing posture depend on Developer account readiness. |
| `ios-revenuecat-product-mapping-run` | 3 | HIGH | Required for paid iOS functionality, but App Store Connect and RevenueCat console work should remain a separate approved high-risk run. |
| `ios-push-apns-firebase-readiness-run` | 4 | HIGH | Needed for iOS push, but APNs capability, Firebase iOS app, plist, and entitlements depend on signing/capability readiness. |
| `ios-first-local-build-readiness-run` | 5 | HIGH | Direct build path, but likely blocked until signing/Team ID/provisioning questions are answered. |
| `ios-generated-output-policy-run` | 6 | MEDIUM | Useful repo hygiene follow-up, but not the most direct TestFlight blocker. |

### Selected Next Run Candidate

| Field | Value |
| --- | --- |
| Selected candidate | `ios-xcode-signing-readiness-run` |
| Reason | The iOS platform now exists, so the next practical blocker is whether Xcode signing, Team ID, Bundle ID, provisioning, and capability readiness are known well enough to support later local build/archive/TestFlight work. |
| Why not auto-open it | This task only selects the next candidate. The next active-run should be created explicitly with its own guardrails. |

### Excluded Candidates And Reasons

| Candidate | Exclusion reason |
| --- | --- |
| `ios-auth-apple-signin-risk-run` | High review risk, but implementation/capability work should follow signing and Bundle ID readiness audit. |
| `ios-revenuecat-product-mapping-run` | Requires App Store Connect/RevenueCat decisions and likely console changes, so it should remain a separate approved high-risk run. |
| `ios-push-apns-firebase-readiness-run` | Push capability depends on APNs/Firebase/iOS entitlement setup, which should be sequenced after signing/capability readiness. |
| `ios-first-local-build-readiness-run` | Too early while signing/Team ID/provisioning are unresolved. |
| `ios-generated-output-policy-run` | Lower direct TestFlight impact than signing readiness. |

### Next Run Guardrail

Recommended scope for `ios-xcode-signing-readiness-run`:

- Audit Xcode project signing state, Team ID need, Bundle ID registration need, provisioning need, and capability candidates.
- Document required Apple Developer/App Store Connect/Xcode inputs before any build/upload.
- Do not modify signing, Bundle ID, `project.pbxproj`, `Info.plist`, entitlements, or capabilities without explicit approval.
- Do not run `npx cap sync ios`, Xcode, pod install, iOS build/archive/upload, or external console changes.
- Keep Sign in with Apple implementation, RevenueCat/App Store product mapping, APNs/Firebase push setup, and TestFlight upload in separate scoped runs.

...
- 상태: `STOPPED_PRE_CHANGE / RELEASE_CONFOUNDED / INCONCLUSIVE`.
- 기준선 시각: 2026-08-01 16:09 KST. 이 시점의 운영 기준선은 가입자 73명, `legacy_beta` 12명, 실제 유료 구독 0명이다.
- 고유 사용자 퍼널은 Pro gate 40명, paywall 12명이며 두 단계를 순서대로 거친 사용자는 10명(약 25%)이다. 구매 시작과 검증된 유료 권한 활성화는 0명이다.
- 기존 12명은 첫 주 활성화가 0명이지만, 2026-07-20부터만 이벤트가 쌓였고 관찰 중 제품·계측 계약이 바뀌었으므로 가격 또는 PMF의 결론으로 사용하지 않는다.
- 16:09 KST 이후 운영 UI 감사에서 생성된 이벤트 12건은 삭제하지 않는다. `internal` QA로 보존하고 Coin Pro v2 전환 집계에서는 제외한다.
- 기존 `legacy_beta` 권한은 회수하지 않는다. 다만 새 14일 체험·유료 전환 cohort와 모든 v2 분모에서 제외한다.
- 아래의 기존 관찰 창과 명령은 역사적 재현용으로 보존하며 최종 성공/실패 판정에는 사용하지 않는다.

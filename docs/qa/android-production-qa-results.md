...
| Execution date | `TBD` |
| Executor | `TBD` |
| Target app version | `TBD` |
| Target commit | `TBD` |
| Test device | `TBD` |
| Android OS version | `TBD` |
| Install path | `TBD` - Play Store / internal / local build / other |
| QA account state | `TBD` - signed out / Basic / Pro / unknown |
| QA scope | Android production automatic smoke, manual device QA, and read-only Play Console review. |
| Overall result | `NOT_RUN` |
| Summary counts | PASS: `0`; FAIL: `0`; BLOCKED: `0`; NOT_RUN: `0`; NEEDS_RUN: `0` |
| Protected areas touched? | `No` until proven otherwise. |
| Notes | `TBD` |

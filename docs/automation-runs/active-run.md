...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | TODO | `/alerts` 현재 박스 구조 조사 | Alerts UI audit | LOW | `/alerts`, `RadarAlertCenter`, `UsageMeterPanel` 등에서 `AppSurface`, `PanelCard`, `bg-ui-inset`, `border`, `rounded`, `shadow` 사용 구조를 조사한다. | 코드 수정 금지. | `git diff --check` |

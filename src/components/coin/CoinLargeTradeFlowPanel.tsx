...
export function CoinLargeTradeFlowPanel({ mode }: { mode: LargeTradeMode }) {
  ...
  const loadReports = useCallback(async () => {
    ...
    setStatus("error");
    setError("큰 체결 흐름을 잠시 확인하지 못했습니다.");
  }, [symbols]);
  ...
  const topSummary = topCard
    ? `${topCard.label} ${sideLabel(topCard.report.dominantSide)} · ${formatUsd(topCard.report.totalLargeNotionalUsd)}`
    : isAltMode
      ? "알트 큰 체결 흐름을 확인하는 중입니다."
      : "메이저 큰 체결 흐름을 확인하는 중입니다.";

  return (
    <PanelCard variant="report" padding="md" className="space-y-4 border-y border-ui-line">
      <SectionHeader
        eyebrow="Binance 공개 선물 체결"
        title={isAltMode ? "알트 큰 체결 흐름" : "메이저 큰 체결 흐름"}

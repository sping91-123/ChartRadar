...
const cftcPositioning = read("src/lib/server/news/cftcPositioning.ts");
const newsImpactSort = read("src/lib/newsImpactSort.ts");
...
const newsOperationalHardeningMigration = read("supabase/migrations/20260724024500_news_impact_operational_hardening.sql");
...
const homeInterestCoinSettingsDialog = read("src/components/coin/HomeInterestCoinSettingsDialog.tsx");
const perpetualDecisionExperience = read("src/components/coin/PerpetualDecisionExperience.tsx");
const perpetualAnalysisWorkspace = read("src/components/coin/PerpetualAnalysisWorkspace.tsx");
const perpetualEvidenceWorkbench = read("src/components/coin/PerpetualEvidenceWorkbench.tsx");
const perpetualTechnicalEvidencePanel = read("src/components/coin/PerpetualTechnicalEvidencePanel.tsx");
...
expectIncludes(officialNewsAdapters, "response.url || url", "공식 출처 redirect 최종 host 재검증", "src/lib/server/news/officialSourceAdapters.ts");
expectIncludes(cftcPositioning, "cftc_positioning_observations", "CFTC 주간 자료 cron 저장 원장", "src/lib/server/news/cftcPositioning.ts");
expectNotIncludes(cftcPositioning, "const cache = new Map", "CFTC 사용자 요청별 메모리 fetch 제거", "src/lib/server/news/cftcPositioning.ts");
...
expectIncludes(newsImpactPanel, "Pro는 발표 전후를 같은 기준으로 비교합니다", "Basic 잠긴 발표 전후 비교 미리보기", "src/components/news/NewsImpactPanel.tsx");
expectIncludes(newsImpactPanel, "발표 당시 판단과 반응을 복기에 저장", "Pro NEWS 복기 연결", "src/components/news/NewsImpactPanel.tsx");
expectIncludes(newsImpactSort, 'event.reactionEligibility === "context_only"', "context-only 규제 문서 대표 사건 차단", "src/lib/newsImpactSort.ts");
...
expectIncludes(newsImpactOutbox, "claim_news_impact_delivery_tokens", "뉴스 FCM token 전송 전 원자적 시도 선점", "src/lib/server/news/newsImpactAlertOutbox.ts");
...
expectIncludes(newsOperationalHardeningMigration, "delivery_attempted_token_ids", "뉴스 Push at-most-once 원장", "supabase/migrations/20260724024500_news_impact_operational_hardening.sql");
...
expectIncludes(homeInterestCoinSettingsDialog, "/api/crypto-exchange-markets?", "Home exchange market list API source", "src/components/coin/HomeInterestCoinSettingsDialog.tsx");
...
expectIncludes(homePerpetualDecisionFlow, "HomeInterestCoinSettingsDialog", "Home coin settings moved beside the analysis tabs", "src/components/coin/HomePerpetualDecisionFlow.tsx");
expectIncludes(homePerpetualDecisionFlow, 'role="tablist"', "Home interest coins use accessible tabs", "src/components/coin/HomePerpetualDecisionFlow.tsx");
expectNotIncludes(homePerpetualDecisionFlow, "/api/crypto/perpetual/monitors?status=active", "Home duplicate monitor usage request removed", "src/components/coin/HomePerpetualDecisionFlow.tsx");
expectNotIncludes(homePerpetualDecisionFlow, "/api/news-impact", "Home duplicate official-news request removed", "src/components/coin/HomePerpetualDecisionFlow.tsx");
expectNotIncludes(homePerpetualDecisionFlow, "HomeDailyActions", "Home duplicate navigation cards removed", "src/components/coin/HomePerpetualDecisionFlow.tsx");
expectIncludes(homeInterestCoinSettingsDialog, 'data-testid="home-interest-settings-dialog"', "Shared Home coin settings dialog", "src/components/coin/HomeInterestCoinSettingsDialog.tsx");
expectIncludes(homeInterestCoinSettingsDialog, 'event.key === "Escape"', "Home coin settings Escape close", "src/components/coin/HomeInterestCoinSettingsDialog.tsx");
expectIncludes(homeInterestCoinSettingsDialog, "recordBasicHomeInterestChange", "Basic coin change is recorded only in the shared save path", "src/components/coin/HomeInterestCoinSettingsDialog.tsx");
...
expectIncludes(perpetualDecisionExperience, "PerpetualAnalysisWorkspace", "Snapshot-bound analysis workspace", "src/components/coin/PerpetualDecisionExperience.tsx");
expectIncludes(perpetualAnalysisWorkspace, "PerpetualEvidenceWorkbench", "Snapshot-bound ICT and combined evidence workbench", "src/components/coin/PerpetualAnalysisWorkspace.tsx");
expectIncludes(perpetualAnalysisWorkspace, "PerpetualTechnicalEvidencePanel", "Snapshot-bound technical evidence workbench", "src/components/coin/PerpetualAnalysisWorkspace.tsx");
...
expectIncludes(perpetualTechnicalEvidencePanel, "multiTimeframeEvidence.find", "Exact-timeframe technical evidence", "src/components/coin/PerpetualTechnicalEvidencePanel.tsx");
